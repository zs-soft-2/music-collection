...
export class AlbumStateServiceImpl extends AlbumStateService {
  ...
  public dispatchChangeNewEntityButtonEnabled(enabled: boolean): void {
    this.store.dispatch(albumActions.changeNewEntityButtonEnabled({ enabled }));
  }
  ...
  public dispatchSetSelectedEntityIdAction(entityId: string): void {
    this.store.dispatch(albumActions.setSelectedAlbumId({ albumId: entityId }));
  }
  ...
  public selectEntityById$(uid: string): Observable<AlbumEntity | undefined> {
    return this.store.pipe(select(albumSelectors.selectAlbumById(), { uid }));
  }
